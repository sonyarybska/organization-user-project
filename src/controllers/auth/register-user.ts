...
export async function registerUser(data: RegisterUserDto): Promise<void> {
  ...
  const { transactionService, cognitoService } = data;
  ...
  await transactionService.run(async (connection) => {
    ...
    const { id } = await data.userRepo.reconnect(connection).create({
      ...userCreateData,
      cognitoUserId
    });

    await data.userOrganizationRepo.reconnect(connection).create({ userId: id, organizationId, role: UserRoleEnum.ADMIN });
  });
}
