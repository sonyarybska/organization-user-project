...
export function getOrganizationRepo(db: DataSource | EntityManager): IOrganizationRepo {
  const organizationRepo = db.getRepository<OrganizationEntity>(OrganizationEntity);
  ...
  return {
    ...
    async getByUserId(userId: string): Promise<OrganizationEntity[]> {
      try {
        ...
        throw new DBError(`Organizations for user with id ${userId} not found`, error);
      }
    },

    async getByIdAndUserId(organizationId: string, userId: string): Promise<OrganizationEntity> {
      try {
    ...
    async create(organization: Partial<Organization>): Promise<OrganizationEntity> {
      try {
        ...
        if (error instanceof QueryFailedError) {
          if (error.message.includes('duplicate key value violates unique constraint')) {
            throw new DBDuplicateError('Organization', error, ErrorCode.DB_ITEM_DUPLICATE);
          }
        }
